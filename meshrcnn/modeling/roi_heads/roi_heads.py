    ...
    def _forward_shape(self, features, instances):
        ...
        if self.training:
            ...
            if self.voxel_on:
                ...
                loss_voxel, target_voxels,pred_voxel_logits = voxel_rcnn_loss(
                ...
                if self.cls_agnostic_voxel:
                    ...
                    with torch.no_grad():
                        vox_in = pred_voxel_logits.sigmoid()  # (N, V, V, V)
                        init_mesh = cubify(vox_in, self.cubify_thresh)  # 1
            ...
            if self.voxel_on:
                ...
                voxel_logits = self.voxel_head(voxel_features)
                pred_voxel_logits = voxel_rcnn_inference(voxel_logits, instances)
                if self.cls_agnostic_voxel:
                    with torch.no_grad():
                        vox_in = voxel_logits.sigmoid().squeeze(1)  # (N, V, V, V)
                        init_mesh = cubify(vox_in, self.cubify_thresh)  # 1
