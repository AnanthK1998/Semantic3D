...
def voxel_rcnn_loss(pred_voxel_logits, instances, loss_weight=1.0):
    ...
    return voxel_loss, gt_voxel_logits,pred_voxel_logits
...
def voxel_rcnn_inference(pred_voxel_logits, pred_instances):
    ...
    return pred_voxel_logits[indices, class_pred]


@ROI_VOXEL_HEAD_REGISTRY.register()
class VoxelRCNNConvUpsampleHead(nn.Module):
    """
    The sheared 3D decoder using the shearing layer.
    """
    ...
    def __init__(self, cfg, input_shape):
        ...
        self.bn_list = []
        self.shearing = ShearingLayer() #12x12x12
        for k in range(num_conv):
            ...
            bn = nn.BatchNorm3d(conv_dims)
            self.add_module("voxel_fcn{}".format(k + 1), conv)
            self.add_module("voxel_bn{}".format(k + 1), bn)
        ...
        self.predictor = nn.Conv3d(
            16, self.num_classes, kernel_size=3, stride=1, padding=1
        )
...
def zero_out_rows(a,number):    #H,W
    mask = torch.zeros(a.shape).cuda()#+learnable
    mask[number]=1
    return a*mask

def stack_up_zeroed_rows(a):   #torch.Size([7, 7])
    output = torch.zeros((a.shape[0],)+a.shape)
    for i in range(a.shape[0]):
        output[i]=zero_out_rows(a,i)
    return output


class ShearingLayer(nn.Module):
    def __init__(self) -> None:
        '''
        Proposed shearing layer to convert 2D feature maps to 3D feature maps.
        '''
        super().__init__()
    def forward(self,input):
        #B,C,W,H
        output = torch.zeros(input.shape + (input.shape[3],)).cuda()
        #B,C,D,W,H
        for i in range(input.shape[0]):
            for j in range(input.shape[1]):
                output[i,j,:,:,:]=stack_up_zeroed_rows(input[i,j,:,:])
        return output